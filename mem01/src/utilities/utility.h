...
template <typename T> void writeAscii(std::string filename, const T *buffer, size_t length) {
...
template <typename T> void readChunks(std::ifstream &f, T *destination, size_t numElements, size_t chunkSize) {
...
template <typename T> constexpr T degToRad(T deg) { return deg * M_PI / 180.0; }
/// Conversion function from radian to degree
template <typename T> constexpr T radToDeg(T rad) { return rad * 180 / M_PI; }

/// Performs a copy between the given buffers while transposing the 2D-matrix,
/// exchanging width and height
template <typename T> void memcpyTransposed(T *dest, const T *src, size_t width, size_t height) {
...
template <typename valueType> std::string stringify(valueType v) {
    std::stringstream ss;
    ss << v;
    return ss.str();
}
...
template <typename valueType> std::string stringify(std::vector<valueType> v) {
    std::string b;
    if (v.size() > 0) {
        b += "[";
...
template <typename valueType, size_t N> std::string stringify(std::array<valueType, N> v) {
    std::string b;
    if (N > 0) {
        b += "[";
        for (valueType &value : v) {
...
int pwLineNumberToFrameNumber(int lineNum);

int pwFrameNumberToLineNumber(int frameNum);

static std::mt19937 mersenneEngine(std::random_device{}());

template <typename IntType> IntType uniform_random_int(IntType a, IntType b) {
